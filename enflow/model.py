...
class ENFlow(nn.Module):
    def __init__(self, node_nf, n_iter, dt, dh, r_cut, temp, loss_norm_const=1e5, device='cpu', act_fn=nn.SiLU(), coords_weight=1.0, norm_diff=False, tanh=False):
        super().__init__()
        self.device = device
        networks = []
        ...
        self.kBT = temp
        self.loss_norm_const = loss_norm_const
    ...
    def reverse(self, data):
        for network in reversed(self.networks):
            data.h = data.h - data.h_vel*self.dh
            data.pos = data.pos - data.vel*self.dt
            edges = self.get_edges(data)
            scaling_factor, force, h_force = network(data.h, edges, data.pos)
            data.h_vel = data.h_vel -h_force*self.dh
    ...
    def nll(self, out, ldj):
        ...
        log_pz = -H/self.kBT - out.N.sum()*self.loss_norm_const - (z*z).sum()*0.5 - math.log(math.sqrt(2*math.pi))
        log_px = ldj + log_pz 
        N_batch = len(out)
        return -log_px/(N_batch*self.loss_norm_const)